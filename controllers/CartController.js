  ...
  db.query(productExistsQuery, [productId], async (err, results) => {
    ...
    db.query(cartCheckQuery, [userId, productId], async (err, cartResults) => {
      ...
      if (cartResults.length > 0) {
        ...
        const updateCartQuery = "UPDATE cart SET quantity = quantity + 1 WHERE userId = ? AND productId = ?";
        ...
        const insertCartQuery = "INSERT INTO cart (userId, productId, quantity) VALUES (?, ?, 1)";
...
exports.getCartItems = catchAsyncErrors(async (req, res, next) => {
  ...
  const sql = `
    ...
    WHERE
      c.UserId = ?
    GROUP BY
      p.ProductId, p.ProductName, p.description, p.Price, p.CategoryName
  `;
